...
let MobileService = MobileService_1 = class MobileService {
    db;
    logger = new Logger(MobileService_1.name);
    constructor(db) {
        this.db = db;
    }
    ...
    async getSales(tenantId, period = 'today') {
    ...
    async getTreasury(tenantId) {
        ...
        const theoreticalCashInHand = Math.max(0, summary.openingFund + summary.totalCashSales + summary.totalCashIn - summary.totalCashOut - summary.totalCashExpenses);
        return {
...
MobileService = MobileService_1 = __decorate([
    Injectable(),
